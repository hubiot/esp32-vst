...
// 測定(MCP3424 / 雨量計 / 統計計算) と 通信(WiFi / SoftAP / AWS IoT MQTT) の統合
// =============================================================================
...
struct para_d {
  int model_no;           // 0: rex noise/vibration, 1: 4ch normal cloud, 2: 4ch normal local, 3: rex rain
...
const char ntp_server[][30] = {"ntp.nict.jp", "pool.ntp.org", "ntp.jst.mfeed.ad.jp"};
long CUR_TIME;
struct tm TIMEINFO;
...
String html_tag1 = "<!DOCTYPE HTML>\r\n<html>\r\n<head>\r\n"
                   ...
                   "  select, input[type=password], input[type=text] { font-size: 16px; padding: 8px; margin: 6px 0; width: 90%; max-width: 320px; box-sizing: border-box; }\r\n"
                   "  button { font-size: 16px; padding: 8px 24px; margin: 10px; border: 1px solid #aaa; border-radius: 4px; cursor: pointer; }\r\n"
...
boolean chk_host_ip(String *str)
{
  ...
  for (int i = 0; i < 4; i++)
  {
    if (!is_number(dst[i])) return false;
    int tmp = dst[i].toInt();
...
void eeprom_write(void)
{
  ...
  for (int i = 0; i < 4; i++)
  {
    strncpy(cfg.s_n_xave_flg[i], PARA.s_n_xave_flg[i].c_str(), sizeof(cfg.s_n_xave_flg[i]) - 1);
  }
...
boolean eeprom_read(void)
{
  ...
  else
  {
    ...
    if (PARA.model_no < 0 || PARA.model_no > 3) PARA.model_no = 0;
    for (int i = 0; i < 4; i++)
    {
    ...
    if (PARA.meas_period < 2) PARA.meas_period = 600;
...
// 100ms / 1000ms 周期 ADCサンプリングとデータ集計
void meas_adc_sample(unsigned long now)
{
  if (now - MEAS_TIME >= SMPL_TIME)
  {
    ...
    if (AP_MODE)
    {
      // APモード時はWebからの要求に応じて値を返すため、周期送信はスキップ
    }
    else
    {
      // 通常モード: 送信タイミング判定
      if (FIRST_FLAG || ((MCNT >= PARA.meas_period * 10 - 1) && PARA.model_no != 3) || RAIN_FLAG)
      {
        int sd_cnt = 0;
        RAIN_FLAG = false;
        if (FIRST_FLAG)
        ...
        // 通信モジュールへ測定データを引き渡して送信
        comm_publish_meas_data(SEND_DATA);
        MCNT = 0;
        for (int i = 0; i < 2; i++) LEQ[i] = 0;
      }
      else
      {
        MCNT++;
...
void comm_publish_meas_data(float *sdata)
{
...
#elif CLOUD_DEBUG == 2
    sprintf(st_ch1, "%s@%s@%s@%s@%s@%s@%s@%s", st_ch4, st_ch4, st_ch4, st_ch4, st_ch4, st_ch4, st_ch4, st_ch4);
...
#elif CLOUD_DEBUG == 4
    sprintf(pub_msg, "{\"id\":\"rx02\",\"ch1\":\"%s\",\"ch2\":\"%s\",\"ch3\":\"%s\",\"ch4\":\"%s\",\"time\":\"%s\"}", st_ch4, st_ch4, st_ch4, st_ch4, st_time);
#else
  ...
  case 2: // ノーマル4ch local
    // ch1
    sprintf(st_ch1, "%.1f", (PARA.s_n_xave_flg[0] == "1") ? sdata[0] : sdata[1]);
    // ch2
    sprintf(st_ch2, "%.1f", (PARA.s_n_xave_flg[1] == "1") ? sdata[10] : sdata[11]);
    // ch3
    sprintf(st_ch3, "%.1f", (PARA.s_n_xave_flg[2] == "1") ? sdata[20] : sdata[21]);
    // ch4
    sprintf(st_ch4, "%.1f", (PARA.s_n_xave_flg[3] == "1") ? sdata[22] : sdata[23]);
...
String HTML_Select_Box_str(String Sel_Ssid)
{
  ...
  str += "  </select><br>\r\n";
  str += "  <a href='/wifi_rescan' style='display:inline-block; padding:4px 10px; margin:4px 0 12px 0; background:#e0e0e0; border-radius:4px; font-size:13px; color:#333;'>再検索 (Rescan)</a><br>\r\n";
  str += "  <label for='pass1'><b>Password:</b></label><br>\r\n";
  str += "  <input type='password' name='pass1' id='pass1'><br>\r\n";
  str += "  <button type='submit' name='ssid_sel_submit' value='send' style='background-color:#AFA; font-weight:bold;'>SET</button>\r\n";
  str += "</form><br><br>\r\n";
...
void html_send(boolean sta_connected, String message1, String message2, String color, String html_res_head, String html_tag1, String html_tag2)
{
  ...
  {
    client.printf("<p style='color:%s; font-size:100%%; font-weight:bold;'>%s</p>\r\n", color.c_str(), message2.c_str());
  }
  if (sta_connected)
  {
...
void wifi_set_submit(String req_str)
{
  ...
  if (getTXT_select > 0)
  {
    ...
    pass_tmp = req_str.substring(req_str.indexOf("&pass1=") + 7, req_str.indexOf("&ssid_sel_submit"));
  }
...
void get_pram_from_url(String req_str)
{
  ...
  if (idx_ch_num > 0)
  {
    ...
    if (is_float(s_conv_param))
    {
      ...
      Serial.printf("Param Set CH%d %s: %s\n", ch_idx + 1, (is_small == 0) ? "LARGE" : "SMALL", s_conv_param.c_str());
    }
  }
}
...
void get_shreshold_from_url(String req_str)
{
  ...
  if (idx0 > 0)
  {
    String stmp = req_str.substring(idx0 + 17, req_str.indexOf("&shreshold_submit"));
    if (is_float(stmp))
    {
...
void get_meas_period_from_url(String req_str)
{
  ...
  if (idx0 > 0)
  {
    String stmp = req_str.substring(idx0 + 19, req_str.indexOf("&meas_period_submit"));
  ...
  if (client)
  {
    ...
    while (client.connected())
    {
      while (client.available())
      {
        ...
        else if (req_str.indexOf("GET /disp_trans_param") >= 0 || req_str.indexOf("GET /get_meas_param") >= 0)
        {
          PAGE_NUM = 1;
        ...
        else if (req_str.indexOf("GET /host_ip_set/?") >= 0)
        {
          ...
          if (idx_host_ip > 0)
          {
            String stmp = req_str.substring(idx_host_ip + 15, req_str.indexOf("&host_ip_para_submit"));
            if (chk_host_ip(&stmp))
            {
              PARA.host_ip = stmp;
              eeprom_write();
        ...
        else if (req_str.indexOf("GET /disp_ave_normal") >= 0)
        {
          client.print(html_res_head2);
          String stmp = "";
          for (int i = 0; i < 4; i++) stmp += PARA.s_n_xave_flg[i] + ",";
          client.print(stmp.c_str());
          delay(10);
          client.stop();
        }
        ...
        {
          pre_url = "GET /f1c9t";
          String stmp = req_str.substring(req_str.indexOf("GET /?model_no=") + 15, req_str.indexOf("&factory_param_submit"));
        ...
        else if (req_str.indexOf("GET /") >= 0)
        {
          ...
          switch (PARA.model_no)
          {
          case 0: client.print(str_rex_noise_shake); break;
          case 1: client.print(str_normal_4ch_cloud); break;
          case 2: client.print(str_normal_4ch_local); break;
          case 3: client.print(str_rex_rain); break;
          default: break;
          }
          delay(10);
          client.stop();
        ...
        else
        {
          ...
          if (pre_url.indexOf("GET /param_set") >= 0) client.print(str_calibration);
          else if (pre_url.indexOf("GET /meas_period_set") >= 0) client.print(str_meas_period);
          else if (pre_url.indexOf("GET /host_ip_set") >= 0) client.print(str_host_ip);
          else if (pre_url.indexOf("GET /f1c9t") >= 0) client.print(str_factory);
          else if (pre_url.indexOf("GET /ave_normal_set") >= 0) client.print(str_ave_normal);
          else if (pre_url.indexOf("GET /shreshold_set") >= 0) client.print(str_shreshold);
          else client.print(str_normal_4ch_cloud);
          delay(10);
          client.stop();
        }
      }
...
void disp_info(void)
{
  ...
  switch (PARA.model_no)
  {
  case 0: Serial.println("NOISE/VIBRATION (CH1:noise, CH2:vibration, CH3:ave, CH4:ave)"); break;
  case 1: Serial.println("Normal 4ch cloud"); break;
  case 2: Serial.printf("Normal 4ch local (Host IP: %s)\n", PARA.host_ip.c_str()); break;
  case 3: Serial.println("RAIN"); break;
  }
